function FinalFantasy2(BattleFild)

NJcontrolBattleFild = BattleFild;
...
for i = 1:12
    ...
    [sta,BattleFildPath] = staGerator(size(Fbuffer,2)+1,TimeFild4C,3);
    ...
    if(Fbuffer(3,t) == 1)
        ...
        BattleFild(1,k) = BattleFild(1,k)*1.1;
        while(k~= i+8)
            count = count + 1;
            %plot( [mapss(k,1) mapss(BattleFildPath(k,i+8),1)],[mapss(k,2) mapss(BattleFildPath(k,i+8),2)],'-g');
            BattleFild(k,BattleFildPath(k,i+8)) = BattleFild(k,BattleFildPath(k,i+8));
